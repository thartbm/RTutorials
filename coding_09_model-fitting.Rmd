...
for (dfno in c(1,2)) {
  ...
  plot(cos(seq(0,pi/2,(pi/2)/100))*10, sin(seq(0,pi/2,(pi/2)/100))*10, type='l', col='#AAAAAA', main=c('aligned','misaligned')[dfno], xlab='x [cm]', ylab='y [cm]', xlim=c(-2,12),ylim=c(-2,12),bty='n', ax=F,asp=1)
...
if ('optimx' %in% installed.packages()) {
  ...
  cat('optimx is NOT installed, please install it before coninueing with the next exercise\n')
}
```
...
We saw that there different methods to find the optimal parameters. What are the differences between those methods? Why would you pick one over the other? The differences lie in _how_ the methods search the (multi-dimensional) parameter space. In the example we've been using so far, the parameter space is simple: x and y coordinates for a central value. That is, there are 2 parameters that are on the same scale and should be about equally important to the quality of the fit. This probably makes the search thorugh the parameter space fairly straightforward.

[turns out this is more about parameter space and going towards normalizing parameters? this should probably come later]
...
res <- microbenchmark('Nelder-Mead' = optimx( par = c( 'x'=0.1, 'y'=0.1), 
                      ...
                      'L-BFGS-B' = optimx( par = c( 'x'=0.1, 'y'=0.1), 
                                           fn = circleFitError, 
                                           lower = c(-5,-5), upper=c(5,5), # we now have to specify boundaries!
                                           method='L-BFGS-B', 
                                           coords = miscoords, radius = 10),
                      times = 100
               )
...
As we could see when fitting the model with very incorrect starting parameters, sometimes the algorithm doesn't converge on a good estimate. A good choice of starting parameters is therefore also important. For the circle fitting problem, you could say that the average of the x-coordinates that are in the data and the average of the y-coordinates that are in the data, would always avoid a wrong set of starting parameters:
...
Such incorrect locations are called 'local minima'. The presence of local minima is not always obious or detectable, especially with models where the parameters interact in sometimes unpredictable ways. One way to get out of this is to do a grid search first.
...
```{r}
xvalues <- seq(min(miscoords$x),max(miscoords$x),diff(range(miscoords$x))/3) # three values for x
yvalues <- seq(min(miscoords$y),max(miscoords$y),diff(range(miscoords$y))/3) # three values for y
searchgrid <- expand.grid('x'=xvalues, 'y'=yvalues) # all 16 combinations
kable(searchgrid)
```
...
The second row shows a local minimum, but the first and third row are pretty good (perhaps equally good), and with very similar x and y coordinates. We need to pick the row with the lowest number in the column `value`:
...
If there is a tie, you might use the number of function evaluations to break it (lower is better?) but that should be rare. In essence we get the same fitted circle centre as we got several times earlier. Let's see how well that circle works on this data:
...
for (dfno in c(1,2)) {
  ...
  lines((cos(seq(0,pi/2,(pi/2)/100))*10)+win$x, (sin(seq(0,pi/2,(pi/2)/100))*10)+win$y, col='red')
  axis(side=1,at=c(0,5,10))
  axis(side=2,at=c(0,5,10))
}
```
...
You are now ready to implement a more useful model.


# Model quality

Usually, what we will do is test how well one given model explains data from two different conditions or test how well two different models explain the same data. This requires model evaluation, and this is a statistics question, and hopefully, we'll add a tutorial to the statistics section about this soon.