...
For most analyses in the paper, we did not use the 15 degree point, as it had perhaps too much missing data at exactly that hand angle. This means that a few participants might have biased the analyses, so that way we avoided that impression. Maybe that was a bad choice, but it's what we did.
...
Some might think that is acceptable, but it does add up to over 1/5th of the data. And that's not random missing data: it's 1 of 7 hand angles and 3 of 21 participants that no longer contribute any data. Not super-kosher...

I don't want to loose any of the levels of the other factors. So we should probably do some other analysis.

# Linear Mixed Effects models

Linear mixed effects models to the rescue!

First, if you want to reproduce the analyses that were published, you should set `remove15` to `TRUE` in the following chunk:
...
So let's fit the linear mixed effects model:
...
The `lmerTest` package allows using the "Satterthwaite" method and overwrites the `anova` function:
