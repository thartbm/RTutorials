...
twoRateMSE <- function(par, schedule, reaches) {
  ...
  return( sqrt( mean((twoRateModel(par, schedule)$total - reaches)^2, na.rm=TRUE) ) )
...
twoRateMSE <- function(par, schedule, reaches) {
  ...
  bigError <- sqrt(mean(schedule^2, na.rm=TRUE)) * 10
...
The MSE `value` is very similar to the one we had before without grid search (2.29). Perhaps this means we didn't need grid search after all, but with this model there is a 4-dimensional parameter space and no obvious way to find where local minima are, so it is better to do grid search.
