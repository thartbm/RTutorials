...
But the criterion function will be different:
...
twoRateLikelihood <- function(par, schedule, reaches) {
  ...
  # the following line works both is sd is a single value or a vector:
  likes <- dnorm((twoRateModel(par, schedule)$total - reaches$mean), mean=0, sd=reaches$sd)
  # print(likes)
  return( prod(likes) )
...
Likes <- apply(searchgrid, FUN=twoRateLikelihood, MARGIN=c(1), schedule=schedule, reaches=reaches)
...
allfits <- do.call("rbind",
                   apply( searchgrid[order(Likes, decreasing = T)[1:10],],
                          MARGIN=c(1),
                          FUN=optim,
                          ...
                          control=control ) )
# pick the best fit:
win <- allfits[order(as.numeric(allfits[,2]), decreasing = T),]
print(win[1])
```
