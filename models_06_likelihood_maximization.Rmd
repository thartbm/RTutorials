...
# it might be a better way to fit the processes, 
# but it's less comparable to MSE, as it effectively
# weighs errors on certain trials differently from others
...
```{r}
likelihoods <- matrix(data=NA, nrow=164, ncol=91)
for (trial in c(1:length(reaches$mean))) {
  likelihoods[trial,] <- dnorm(c(-45:45), mean=reaches$mean[trial], sd=reaches$sd)
}
...
persp(c(1:164), c(-45:45), likelihoods*300, phi = 50, theta = 0,
...
The likelihood is different on the 8th decimal place, which I'm sure we can ignore.
